...
Mapiator.StdTile = function(x, y, z, ox, oy, map) {
	...
	s.left = ''+ this.offsetX +'px';
	s.top = ''+ this.offsetY +'px';
	map.getTileUrl(x,y,z,{
		success : function(o) {
			this.url = o.url;
			this.domElement.style.background = 'url('+ o.url +')';
		},
		failure : function(o) {
		},
		scope:this
...
Mapiator.TileLayer = function(map, visibleArea, TileConstructor){
	...
	this.removeAllTilesNotContainedIn = function( hash ) {
		for( id in tileCache ){
			if( !hash[id] ){
...
Mapiator.TraditionalController = function( map ) {
	...
	/*setTimeout( function() {
		...
		util.addCssRule( '#mapiator_zoom_in', 'background:url(' + map.imgUrlLoc + '/zoomIn_blur.png); top:15px;' );
		util.addCssRule( '#mapiator_zoom_out', 'background:url(' + map.imgUrlLoc + '/zoomOut_blur.png); top:55px;' );
	}, 1);*/
};
