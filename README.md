...
- Do **not** `require('jquery')`! Instead `require('js/shims/jquery')`. We have to do this because we need to manually attach the Semantic UI JS code to the jQuery object. The details aren't really important, just remember to require the right one. :grinning:
